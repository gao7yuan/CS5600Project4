...
bool dir_add(struct dir *dir, const char *name, block_sector_t inode_sector) {
  ...
  /* Set the parent sector of the new node. */
...
bool dir_remove(struct dir *dir, const char *name) {
  ...
  /* If the directory to be deleted is used by other processes. */
  if (inode_is_dir(inode) && (inode_get_open_cnt(inode) > 1)) {
    goto done;
  }

  /* If the directory to be deleted is not empty*/
...
bool dir_has_parent(struct dir *dir, struct inode **inode) {
  ...
  sector = inode_get_parent(dir_get_inode(dir));
  if (sector == -1) {
    return false;
  }
...
bool dir_is_empty(struct inode *inode) {
  ...
  while (inode_read_at(inode, &entry, sizeof(entry), offset) == sizeof(entry)) {
    offset += sizeof(entry);
    if (entry.in_use) {
      return false;
    }
...
bool dir_is_root(struct dir *dir) {
  /* Checks if the given directory is NULL. */
  if (!dir) {
    return false;
  }
